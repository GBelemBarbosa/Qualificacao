...
@with_kw mutable struct ANHSPGModel <: AbstractCompositeSolver
    method = :ANHSPG
...
function ANHSPG(ProblemModel:: AbstractCompositeModel, SolverModel:: ANHSPGModel)
    @unpack_ANHSPGModel SolverModel
    if isnothing(x₀)
        x₀ = ProblemModel.x₀
    end
    ...
    T₀ = time()
    x_best = ryₖ = vₖ = yₖ₋₁ = yₖ = zₖ = vₗₐₛₜ = xₖ₋₁ = xₖ = x₀
    ...
    while status == :running
        ...
        while true
            ...
            i == 1 && k != 1 ? αₖ = nsyₖ₋₁/(αₖ*ryₖ'ryₖ) : αₖ *= ρ
        ...
        if Fzₖ+β*nzₖᵢ₋yₖ/(2*αₖ) <= Fxₗ₍ₖ₎
            ...
            while true
                ...
                i == 1 && k != 1 ? γₖ = nsxₖ/(γₖ*rxₖ'rxₖ) : γₖ *= τ
...
function ANHSPGopt(ProblemModel:: AbstractCompositeModel, SolverModel:: ANHSPGModel)
    @unpack_ANHSPGModel SolverModel
    if isnothing(x₀)
        x₀ = ProblemModel.x₀
    end
