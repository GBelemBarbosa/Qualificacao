...
function newAPG_vs(f:: Function, h:: Function, ∇f:: Function, prox:: Function, Q:: Function, E:: Function, x₀:: Array{<:Number}, λ₁:: Number, μ₀:: Number, μ₁:: Number, c:: Number, δ:: Number, kₘₐₓ:: Int64; ϵ=eps(), p=Inf) 
    ...
    λₖ₊₁=λ₁
    tₖ₊₁=1.0
    xₖ, xₖ₊₁=x₀, prox(λ₁, x₀, zeros(length(x₀[1]))) # prox_λ₁h(x₀)
    ∇fxₖ₊₁=∇f(xₖ₊₁)
    fxₖ₊₁=f(xₖ₊₁)
    Fxₖ₊₁=fxₖ₊₁+h(xₖ₊₁)
    if Fxₖ>Fxₖ₊₁
    ...
    while true
        ...
        if ⎷nψₖ<ϵ
            return F_best, time()-T₀, pr₌gr+1, pr₌gr
        elseif k==kₘₐₓ
...
function newAPG_vsopt(f:: Function, h:: Function, ∇f:: Function, prox:: Function, Q:: Function, E:: Function, x₀:: Array{<:Number}, λ₁:: Number, μ₀:: Number, μ₁:: Number, c:: Number, δ:: Number, kₘₐₓ:: Int64; ϵ=eps(), p=Inf) 
    pr₌gr=0
    Tᵥ=0.0 # Possible time spent on ∇f(xₖ₊₁) (doesn't happen each iteration)
