...
begin
	...
	using LaTeXStrings
	# Leitura ds problemas do Data-Lasso
	using MAT 
...
md"# Experimento com o modelo $\ell_2-\ell_0$ em 25 problemas reais
## Pacotes e *seed*"
...
`params` é a especificação manual dos parâmetros dos métodos, e é opcional.  No exemplo acima, somente 
`nome_parametro_1` e `nome_parametro_2` recebem valores do usuário, e os demais continuam com valores *default*.
...
Especifique sua localização dos dados 👇 (mantenha `/` no final).
"""

# ╔═╡ a33d9e7f-af7d-4390-9d44-3da3996c7946
@bind lassodir TextField((60, 1), default="your_path_to_Data-Lasso/Data-Lasso/")
...
`ϵ` é a tolerância desejada para o critério de parada ``\|\psi_k\|_p<\epsilon``, sendo `p` a norma escolhida abaixo 👇.
"""

# ╔═╡ da957f13-6c02-4b1b-8fce-5a95527583f3
ϵₐₗₗ = 10.0^exp; @bind p Select([Inf => "∞", 1 => 1, 2 => 2])

# ╔═╡ 995fcfbe-0825-49b9-8550-e22484acf723
for i=1:25
    # Leitura dos arquivos dos problemas
    problem_s = "SC$i"
    vars      = matread("$lassodir$problem_s.mat")

    println("$problem_s:")

    A, b, λ₁ = vars["A"], vec(vars["b"]), vars["lambda"] # λ₁ da norma l₁, descartável 

    # O struct Problem_l2l0 define algumas variáveis e funções do problema quadrático
    problem = Problem_l2l0(A=A, b=b) # Gera as funções e variáveis genéricas do problema quadrático ℓ₀
    ϵ       = problem.L*ϵₐₗₗ# Critério de parada
...
md"""
Adicione o nome do experimento 👇.
"""
...
begin
	pltF = performance_profile(PlotsBackend(), F_hist, method_names; palette = :darktest, linewidth = 3)
...
begin
	pltT = performance_profile(PlotsBackend(), T_hist, method_names; palette = :darktest, linewidth = 3)
...
begin
	pltpr = performance_profile(PlotsBackend(), pr_hist, method_names; palette = :darktest, linewidth = 3)
...
begin
	pltgr = performance_profile(PlotsBackend(), gr_hist, method_names; palette = :darktest, linewidth = 3)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "047ed3eeefe10691489e65469ed490fd650cb155"
