...
begin
	...
	# Para o cálculo de Lf e amostragem aleatória dos dados
	Random.seed!(1)
end;
...
md"# Espectrografia de interferograma incompleto
## Pacotes e *seed*"

# ╔═╡ b2cb01a2-3cfa-4a88-9a06-e8dbf1f19175
md"## Leitura dos dados
Dowload dos problemas pode ser efetuado em **[Interferograms](https://drive.google.com/file/d/1pCuItFylT0SgnyVvWyhIel2Tm8Rf5viC/view?usp = drive_link)**.

Escolha o arquivo .csv contendo os dados do interferograma 👇."

# ╔═╡ a33d9e7f-af7d-4390-9d44-3da3996c7946
@bind data_file FilePicker()
...
# ╔═╡ b04455a5-c815-4240-9046-e19dee864c75
md"
Escolha a porcentagem de valores do interferograma `% =` $(@bind percent Scrubbable(0:0.05:1; default = 0.4)).
"

# ╔═╡ 88f96c7c-97df-4de4-a1a5-b53a7381fff9
md"""
A variável `kₘₐₓ =` $(@bind kₘₐₓ Scrubbable(100:100:10000; default = 1000)) determina o número máximo de iterações para os métodos.
...
function read_CSV(nist_csv_file:: Dict; plot_toggle = true)
    df_nist_data    = UInt8.(nist_csv_file["data"]) |> IOBuffer |> CSV.File |> DataFrame
...
# ╔═╡ cc6bd68d-ca5f-486d-b0ba-81a20b62a189
begin
	frequency_range, spectrum_data, interferogram_data, n, plt_data = read_CSV(data_file)
	t_data = eachindex(interferogram_data)
...
function get_m_samples(interferogram:: Vector{<:Number}, m:: Int64; plot_toggle = true)
    ...
    # m amostras aleatórias retiradas de t
    ...
    return sampleₘ, interferogramₘ 
end;

# ╔═╡ 90e1c987-9c5c-4bf0-a963-4912ad699ffa
begin
	m = round(Int64, n*percent)

	sampleₘ, interferogramₘ, plt_samples = get_m_samples(interferogram_data, m)
...
begin 
	...
	function f(x:: Vector{<:Number}; expt = expt, interferogram = interferogramₘ, n = n, n₂ = n₂, m = m, isNyquist=isNyquist)
        ...
        return norm(aux.-interferogram)^2/2
    end
end;

# ╔═╡ ac1be130-a787-4f69-9fc6-ba870aad628c
begin 
	# Este L é igual a eigmax(iDFT'iDFT) quando as amostras estão cheias (0:n-1) para a matriz iDFT cheia (# de amostras = n) usada implicitamente que tem todas as colunas não nulas e não Nyquist = 2*real.(elas mesmas). 
...
function reconstruct_interferogram(t_data:: Union{Vector{<:Number}, Base.OneTo{Int64}}, t_sample:: Vector{<:Number}, spectrum_sample:: Vector{<:Number}, interferogram_data:: Vector{<:Number})
    n         = length(interferogram_data)
    isNyquist = iseven(n)

    expt                 = exp.((2*π*im/n).*t_sample)
    v                    = expt./n
    interferogram_sample = [real(spectrum_sample[begin])/n for i = 1:m]
	
    for i = 2:length(spectrum_sample)-isNyquist
        interferogram_sample .+= 2 .*real.(v.*spectrum_sample[i])
        v                    .*= expt
    end
    if isNyquist
        interferogram_sample .+= real.(v.*spectrum_sample[end])
    end

    plot(t_data, real.(interferogram_data), lw =2, alpha = .5, label = "Interferogram data", dpi = 600)
    scatter!(t_sample, interferogram_sample, label = "Interferogram from samples")
    plot!(title = "Interferogram from irfft of data vs.\nreconstructed from inverse problem spectrum", titlefont = font(7), xlabel = "Optical path [cm]", ylabel = "Volts [V]")
end;
...
function plot_reconstruction(frequency_range:: Vector{<:Number}, spectrum_sample:: Vector{<:Number}, spectrum_data:: Vector{<:Number}, t_sample:: Vector{<:Number}, t_data:: Union{Vector{<:Number}, Base.OneTo{Int64}}, interferogram_data:: Vector{<:Number}) 
    plot(plot_reconstructed_spectrum(frequency_range, spectrum_sample, spectrum_data), reconstruct_interferogram(t_data, t_sample, spectrum_sample, interferogram_data))
end;
...
trigger ? begin
	problem_model = CompositeModel(F = F, f = f, h = h, ∇f = ∇f, prox = prox, x₀ = x₀, optimizable = false)
	solver        = composite_solver(method)
	solver.ϵ      = ϵ
	solver.kₘₐₓ   = kₘₐₓ
	
	
	stats[1] = solve_composite(problem_model, solver) 
end : nothing
